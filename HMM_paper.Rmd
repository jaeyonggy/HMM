...
where, $A$: transition probability matrix (TPM) and $B$: emission probability matrix
...
**_Forward probability_**:

Each cell of the forward algorithm trellis $\alpha_t(j)$ is called a **forward probability** and it represents **the probability of being in state $j$ after seeing the first $t$ observations, given $\lambda = (A,B)$** (where, $A$: transition probability matrix (TPM) and $B$: emission probability matrix).
...
where, $\alpha_1(1)$: the previous forward path probability, $P(H|C)$: the transition probability (A), and $P(1|H)$: the emission probability (B), and
...
where, $\alpha_1(2)$: the previous forward path probability, $P(H|H)$: the transition probability (A), and $P(1|H)$: the emission probabilty (B).
...
hmm = initHMM(
  ...
  emissionProbs=matrix(c(.5,.4,.1,.2,.4,.4),nrow=2,byrow=T)
)
print(hmm)
```
...
The third and the last problem for HMMs is learning the parameters of an HMM, that is, $\lambda = (A,B)$ (where, $A$: the transition probability matrix (TPM) and $B$: the emission probability matrix).
...
<details>
  <summary>Click for summarized view</summary>
    ![](/Users/jaeyonglee/Documents/College/RStudio/HMM/HMM/images_paper/IMG_1287.jpeg)
    ![](/Users/jaeyonglee/Documents/College/RStudio/HMM/HMM/images_paper/IMG_1288.jpeg)
</details> 
...
- $\alpha_t(j)$: **forward probability**; the probability of being in state j after seeing the first t observations, given $\lambda = (A,B)$.

$$
\begin{align*}
\alpha_t(j) &= P(o_1,o_2,\dots,o_t,q_t=j|\lambda) \\ 
 &= \sum_{i=1}^{N} \alpha_{t-1}(j)a_{ij}b_j(o_t)
\end{align*}
$$

\

- $v_t(j)$: **Viterbi probability**; the probabililty that the HMM is in state $j$ after seeing the first $t$ observations and passing through the most probable state sequence $q_1,\dots,q_{t-1}$, given $\lambda=(A,B)$.

$$
\begin{align*}
v_t(j) &= \underset{q_1,\dots q_{t-1}}{\mathrm{max}} P(q_1,\dots,q_{t-1},o_1,\dots,o_t,q_t=j|\lambda) \\ 
 &= \underset{i=1}{\overset{N}{\mathrm{max}}} \ v_{t-1}(i)a_{ij}b_j(o_t)
\end{align*}
$$

\

- $\beta_t(i)$: **backward probability**; the probability of seeing the observations from time $t+1$ to the end, given that we are in state $i$ at time $t$ and given $\lambda=(A,B)$.
...
### 8. Review of dynamic programming
...
![](/Users/jaeyonglee/Documents/College/RStudio/HMM/HMM/images_paper/ss20.png)

\

we can see that the result of fib(2) are needed repeatedly when calculating fib(3) and fib(4), so we should avoid its re-computation by storing its result.
