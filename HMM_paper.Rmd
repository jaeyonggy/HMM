...
To understand the algorithm, it is crucial to define a useful probability related to the forward probability, called the **backward probability**. The backward probability $\beta$ is the probability of seeing the observations from time $t+1$ to the end, given that we are in state $i$ at time $t$ and given $\lambda = (A,B)$.
...
$$
\hat{b}_j(v_k) = \frac{\sum_{t=1}^{T-1}\xi_t(i,j)}{\sum_{t=1}^{T-1}\sum_{k=1}^{N}\xi_t(i,k)} \tag{A.23}
$$
